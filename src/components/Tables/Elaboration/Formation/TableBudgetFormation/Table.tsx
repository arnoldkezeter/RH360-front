...
import { setErrorPageBudgetFormation, setBudgetFormationLoading } from "../../../../../_redux/features/elaborations/budgetFormationSlice";
...
const Table = ({ data, selectedFormation, currentPage, onPageChange, onEdit}: TableBudgetFormationProps) => {
    ...
    const pageIsLoading = useSelector((state: RootState) => state.budgetFormationSlice.pageIsLoading);
    ...
    const itemsPerPage =  useSelector((state: RootState) => state.budgetFormationSlice.data.pageSize); // nombre d'éléments maximum par page
    const count = useSelector((state: RootState) => state.budgetFormationSlice.data.totalItems);
    ...
    const [filteredData, setFilteredData] = useState<BudgetFormation[]>(data);
    

    const latestQueryBudgetFormation = useRef('');
    useEffect(() => {
        dispatch(setBudgetFormationLoading(true));
        latestQueryBudgetFormation.current = searchText;
        
        try{
            
            const filterBudgetFormationByContent = async () => {
                if (searchText === '') {
                    // if(isSearch){
                        ...
                        const result: BudgetFormation[] = data;
                    ...
                    await getFilteredBudgetFormations({page:1, search:searchText, lang, formationId:selectedFormation?._id || ""}).then(result=>{
                        if (latestQueryBudgetFormation.current === searchText) {
                            if(result){
                                ...
                                setFilteredData(budgetFormationsResult);
                            }
                          }
            ...
            filterBudgetFormationByContent();
        }catch(e){
            dispatch(setErrorPageBudgetFormation(t('message.erreur')));
        }finally{
            if (latestQueryBudgetFormation.current === searchText) {
                dispatch(setBudgetFormationLoading(false)); // Définissez le loading à false après le chargement
            }
        }
                    ...
                    {isDropdownVisible && (
                        <div className="flex flex-col justify-start items-start overflow-y-scroll pb-2 h-[400px] gap-y-3">
                            ...
                            <div className="w-full">
                                <InputSearch 
                                    hintText={t('recherche.rechercher')+t('recherche.budget_theme')} 
                                    value={searchText} 
