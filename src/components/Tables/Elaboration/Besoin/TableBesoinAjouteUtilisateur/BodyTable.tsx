...
const BodyTable = ({ data, onEdit}: { data: BesoinAjouteUtilisateur[], onEdit: (besoinAjouteUtilisateur: BesoinAjouteUtilisateur) => void }) => {
        ...
        {data && data.map((item, index) => (
            <tr key={index + 1} className="font-medium text-black dark:text-white text-[12px] md:text-[14px]">
                ...
                <td className="border-b border-[#eee] py-2 lg:py-4 px-4 dark:border-strokedark bg-gray-2 dark:bg-black align-top">
                    <div className="min-h-[40px] flex items-center">
                        <h5>{lang === 'fr' ? item.pointsAAmeliorerFr : item.pointsAAmeliorerEn}</h5>
                    </div>
                </td>
                ...
                <td className="border-b border-[#eee] py-2 lg:py-4 px-4 dark:border-strokedark align-top">
                    <div className="min-h-[40px] flex items-center">
                        <h5>{lang==='fr'?(statuts.find(s=>s.key===item.statut)?.nomFr||""):(statuts.find(s=>s.key===item.statut)?.nomEn||"")}</h5>
                    </div>
                </td>
