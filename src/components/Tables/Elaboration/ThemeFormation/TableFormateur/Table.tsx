...
import { setErrorPageFormateur, setFormateurLoading } from "../../../../../_redux/features/elaborations/formateurSlice";
import { getFilteredFormateurs } from "../../../../../services/elaborations/formateurAPI";



interface TableFormateurProps {
    data: Formateur[];
...
const Table = ({ data, selectedTheme, currentPage, onPageChange, onEdit}: TableFormateurProps) => {
    const {t}=useTranslation();
    const dispatch = useDispatch();
    const [isDropdownVisible, setIsDropdownVisible] = useState(false);
    const lang = useSelector((state: RootState) => state.setting.language); // fr ou en
    const pageIsLoading = useSelector((state: RootState) => state.formateurSlice.pageIsLoading);
    ...
    const itemsPerPage =  useSelector((state: RootState) => state.formateurSlice.data.pageSize); // nombre d'éléments maximum par page
    const count = useSelector((state: RootState) => state.formateurSlice.data.totalItems);
    ...
    const [filteredData, setFilteredData] = useState<Formateur[]>(data);
    

    const latestQueryFormateur = useRef('');
    useEffect(() => {
        dispatch(setFormateurLoading(true));
        latestQueryFormateur.current = searchText;
        
        try{
            
            const filterFormateurByContent = async () => {
                if (searchText === '') {
                    // if(isSearch){
                    ...
                    // setFilteredCycle([]);
                    // setFilteredNiveaux([]);
                    let formateursResult : Formateur[] = [];
                    
                    await getFilteredFormateurs({page:1, search:searchText, lang, themeId:selectedTheme?._id || ""}).then(result=>{
                        if (latestQueryFormateur.current === searchText) {
                            if(result){
                                formateursResult = result.formateurs;
                                setFilteredData(formateursResult);
                            }
                          }
            ...
            filterFormateurByContent();
        }catch(e){
            dispatch(setErrorPageFormateur(t('message.erreur')));
        }finally{
            if (latestQueryFormateur.current === searchText) {
                dispatch(setFormateurLoading(false)); // Définissez le loading à false après le chargement
            }
        }
