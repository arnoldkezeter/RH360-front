...
import { setErrorPageLieuFormation, setLieuFormationLoading } from "../../../../../_redux/features/elaborations/lieuFormationSlice";
import { getFilteredLieuFormations } from "../../../../../services/elaborations/lieuFormationAPI";



interface TableLieuFormationProps {
    data: LieuFormation[];
    selectedTheme?:ThemeFormation;
    currentPage: number;
    onPageChange: (page: number) => void;
    onEdit: (lieuFormation : LieuFormation) => void;
}

const Table = ({ data, selectedTheme, currentPage, onPageChange, onEdit}: TableLieuFormationProps) => {
    const {t}=useTranslation();
    const dispatch = useDispatch();
    const [isDropdownVisible, setIsDropdownVisible] = useState(false);
    const lang = useSelector((state: RootState) => state.setting.language); // fr ou en
    const pageIsLoading = useSelector((state: RootState) => state.lieuFormationSlice.pageIsLoading);
    ...
    const itemsPerPage =  useSelector((state: RootState) => state.lieuFormationSlice.data.pageSize); // nombre d'éléments maximum par page
    ...
    const [filteredData, setFilteredData] = useState<LieuFormation[]>(data);
    

    const latestQueryLieuFormation = useRef('');
    useEffect(() => {
        dispatch(setLieuFormationLoading(true));
        latestQueryLieuFormation.current = searchText;
        
        try{
            
            const filterLieuFormationByContent = async () => {
                if (searchText === '') {
                    // if(isSearch){
                        ...
                        const result: LieuFormation[] = data;
                    ...
                    // setFilteredCycle([]);
                    // setFilteredNiveaux([]);
                    let lieuFormationsResult : LieuFormation[] = [];
                    
                    await getFilteredLieuFormations({page:1, search:searchText, lang, themeId:selectedTheme?._id || ""}).then(result=>{
                        if (latestQueryLieuFormation.current === searchText) {
                            if(result){
                                lieuFormationsResult = result.lieuFormations;
                                setFilteredData(lieuFormationsResult);
                            }
                          }
            ...
            filterLieuFormationByContent();
        }catch(e){
            dispatch(setErrorPageLieuFormation(t('message.erreur')));
        }finally{
            if (latestQueryLieuFormation.current === searchText) {
                dispatch(setLieuFormationLoading(false)); // Définissez le loading à false après le chargement
            }
        }
