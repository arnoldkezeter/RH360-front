...
interface TableChercheurProps {
    ...
    onEtablissementChange:(etablissement:Etablissement)=>void;
    onStatutChange:(statut:Statut)=>void;
    onResetFilters:(value:boolean)=>void;
    onEdit: (chercheur : Chercheur) => void;
}

const Table = ({ data, etablissements, currentPage, currentEtablissement, currentStatut, onPageChange, onEtablissementChange, onStatutChange, onResetFilters, onEdit}: TableChercheurProps) => {
    ...
    const handleEtablissementSelect = (selected: Etablissement | undefined) => {
        if (selected) {
            onEtablissementChange(selected);
        }
    };
    ...
    const itemsPerPage =  useSelector((state: RootState) => state.chercheurSlice.data.pageSize); // nombre d'éléments maximum par page
    const count = useSelector((state: RootState) => state.chercheurSlice.data.totalItems);
    ...
    const [filteredData, setFilteredData] = useState<Chercheur[]>(data);
    

    const latestQueryChercheur = useRef('');
    useEffect(() => {
        dispatch(setChercheursLoading(true));
        latestQueryChercheur.current = searchText;
        ...
        try{
            ...
            const filterChercheurByContent = async () => {
                if (searchText === '') {
                    ...
                    // setFilteredCycle([]);
                    // setFilteredNiveaux([]);
                    let chercheursResult : Chercheur[] = [];
                    
                    await getChercheursByFiltres({page:1, search:searchText, lang}).then(result=>{
                        if (latestQueryChercheur.current === searchText) {
                            if(result){
                                chercheursResult = result.chercheurs;
                                setFilteredData(chercheursResult);
                            }
                          }
            ...
            if (latestQueryChercheur.current === searchText) {
                dispatch(setChercheursLoading(false)); // Définissez le loading à false après le chargement
            }
        }
                    ...
                    {isDropdownVisible && (
                        <div className="flex flex-col justify-start items-start overflow-y-scroll pb-2 h-[400px] gap-y-3">
                            ...
                            <div className="w-full">
                                <InputSearch 
                                    hintText={t('recherche.rechercher')+t('recherche.chercheur')} 
                                    value={searchText} 
                            ...
                            <CustomDropDown2<Etablissement>
                                title={t('label.etablissement')}
                                selectedItem={currentEtablissement}
                                items={etablissements}
                                defaultValue={undefined}
                                displayProperty={(etablissement: Etablissement) => `${lang === 'fr' ? etablissement.nomFr : etablissement.nomEn}`}
