...
import { searchCommune } from "../../../services/settings/communeAPI";
import { setErrorPageCommune, setCommuneLoading } from "../../../_redux/features/settings/communeSlice";
...
interface TableCommuneProps {
    ...
    onDepartementChange:(region:Departement)=>void;
    onCreate:()=>void;
...
const Table = ({data, regions, departements, currentPage, currentRegion, currentDepartement, onPageChange, onRegionChange, onDepartementChange, onCreate, onEdit}: TableCommuneProps) => {
   ...
   const itemsPerPage =  useSelector((state: RootState) => state.communeSlice.data.pageSize); // nombre d'éléments maximum par page
   const count = useSelector((state: RootState) => state.communeSlice.data.totalItems);
   ...
   const endItem = Math.min(count, indexOfLastItem);
    ...
    const [filteredData, setFilteredData] = useState<Commune[]>(data);
    ...
    const latestQueryCommune = useRef('');
    useEffect(() => {
        dispatch(setCommuneLoading(true));
        latestQueryCommune.current = searchText;
        
        try{
            
            const filterCommuneByContent = async () => {
                if (searchText === '') {
                   
                    const result: Commune[] = data;
                    setFilteredData(result); 
                }else{
                    let communesResult : Commune[] = [];
                  
                    await searchCommune({ searchString:searchText, lang:lang}).then(result=>{
                        if (latestQueryCommune.current === searchText) {
                            if(result){
                                communesResult = result.communes;
                                setFilteredData(communesResult);
                            }
                        }
            ...
            filterCommuneByContent();
        }catch(e){
            dispatch(setErrorPageCommune(t('message.erreur')));
        }finally{
            if (latestQueryCommune.current === searchText) {
                dispatch(setCommuneLoading(false)); // Définissez le loading à false après le chargement
            }
        }
        ...
        <div>
            ...
            <div className="flex justify-between items-center gap-x-1 lg:gap-x-2 mb-1 -mt-3 md:mt-0">
                {(roles.admin === userRole || roles.superAdmin === userRole) && <ButtonCreate
                    title={t('boutton.nouvel_commune')}
                    onClick={() => { onCreate();dispatch(setShowModal()) }}
                />}
                <InputSearch hintText={t('recherche.rechercher')+t(t('recherche.commune'))} value={searchText} onSubmit={(text) => setSearchText(text)} />
            </div>
            {/*! bouton creer ajouter un nouvel ... et search bar */}
                    ...
                    {isDropdownVisible && (
                        <div className="flex flex-col justify-start items-start overflow-y-scroll pb-2 h-[200px] gap-x-2 ">
                            ...
                            <CustomDropDown2<Departement>
                                ...
                                displayProperty={(departement: Departement) => `${lang === 'fr' ? departement.nomFr : departement.nomEn}`}
