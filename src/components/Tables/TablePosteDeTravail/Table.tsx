...
import { setErrorPagePosteDeTravail, setPosteDeTravailLoading } from "../../../_redux/features/settings/posteDeTravailSlice";
...
interface TablePosteDeTravailProps {
    ...
    onCreate:()=>void;
    onEdit: (postedetravail:PosteDeTravail) => void;
}
...
const Table = ({data, familleMetiers, currentPage, currentFamilleMetier, onPageChange, onFamilleMetierChange, onCreate, onEdit}: TablePosteDeTravailProps) => {
   ...
   const endItem = Math.min(count, indexOfLastItem);
    ...
    const [filteredData, setFilteredData] = useState<PosteDeTravail[]>(data);
    ...
    const latestQueryPosteDeTravail = useRef('');
    useEffect(() => {
        dispatch(setPosteDeTravailLoading(true));
        latestQueryPosteDeTravail.current = searchText;
        
        try{
            
            const filterPosteDeTravailByContent = async () => {
                if (searchText === '') {
                   
                    const result: PosteDeTravail[] = data;
                    setFilteredData(result); 
                }else{
                    let postedetravailsResult : PosteDeTravail[] = [];
                  
                    await searchPosteDeTravail({ searchString:searchText, lang:lang}).then(result=>{
                        if (latestQueryPosteDeTravail.current === searchText) {
                            if(result){
                                postedetravailsResult = result.posteDeTravails;
                                setFilteredData(postedetravailsResult);
                            }
                        }
            ...
            filterPosteDeTravailByContent();
        }catch(e){
            dispatch(setErrorPagePosteDeTravail(t('message.erreur')));
        }finally{
            if (latestQueryPosteDeTravail.current === searchText) {
                dispatch(setPosteDeTravailLoading(false)); // Définissez le loading à false après le chargement
            }
        }
        ...
        <div>
            ...
            <div className="flex justify-between items-center gap-x-1 lg:gap-x-2 mb-1 -mt-3 md:mt-0">
                {(roles.admin === userRole || roles.superAdmin === userRole) && <ButtonCreate
                    title={t('boutton.nouvel_poste_de_travail')}
                    onClick={() => { onCreate();dispatch(setShowModal()) }}
                />}
                <InputSearch hintText={t('recherche.rechercher')+t(t('recherche.poste_de_travail'))} value={searchText} onSubmit={(text) => setSearchText(text)} />
            </div>
            {/*! bouton creer ajouter un nouvel ... et search bar */}
            ...
            <div className="rounded-sm border border-stroke bg-white px-3 lg:px-5 pt-0 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">     
                <h1 className="text-[12px] lg:text-[15px] mt-3 lg:mt-5 font-medium flex justify-start items-center gap-x-2"><div className="hidden lg:block"><FaFilter /></div>{t('filtre.poste_de_travail')} </h1>
