...
function ModalCreateUpdate({ fonction }: { fonction: CommonSettingProps | null }) {
    ...
    useEffect(() => {
        if (fonction) {
            ...
            setLibelleFr(fonction.libelleFr);
            setLibelleEn(fonction.libelleEn);
    ...
    const handleCreateUpdate = async () => {
        ...
        //update
            ...
            if (!libelleFr || !libelleEn) {
                ...
                //
                // mise a jour
                await apiUpdateFonction(
                    { _id: fonction._id, code, libelleFr, libelleEn }
                ).then((e: ReponseApiPros) => {
                    if (e.success) {
                        createToast(e.message[lang as keyof typeof e.message], '', 0);
