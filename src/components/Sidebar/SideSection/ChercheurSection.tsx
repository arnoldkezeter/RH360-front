...
const ChercheurSidebarLink: React.FC<ChercheurSidebarLinkProps> = ({
    ...
    if (accessibleItems.length === menuItems.length) {
        return (
            <SidebarLinkGroup activeCondition={pathname === '/chercheurs' || pathname.includes('chercheurs')}>
                {(handleClick, open) => (
                    <>
                        <NavLink
                            to="#"
                            ...
                            className={`group relative flex items-center gap-2.5 rounded-sm py-2 pl-3 pr-4 font-medium text-white duration-300 ease-in-out hover:bg-[#1e3a8a] dark:hover:bg-meta-4 ${pathname.includes('chercheurs') ? 'bg-graydark dark:bg-meta-4 text-white' : ''}`}
                        >
                            <div className="w-6">
    ...
    if (accessibleItems.length === 1) {
        ...
        return (
            <li>
                ...
                >
                    <div className="w-6">
                        <HiOutlineLightBulb   className="text-[18px]" />
                    </div>
                    {singleItem.label}
                </NavLink>
            </li>
                ...
                {(handleClick, open) => (
                    <>
                        <NavLink
                            ...
                            className={`group relative flex items-center gap-2.5 rounded-sm py-2 pl-3 pr-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${pathname.includes('chercheurs') ? 'bg-graydark dark:bg-meta-4 text-secondary' : ''}`}
                        >
                            <div className="w-6">
                                <HiOutlineLightBulb   className="text-[22px]" />
                            </div>
...
export default ChercheurSidebarLink;
